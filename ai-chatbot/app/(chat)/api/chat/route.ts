// Modified from vercel/chatbot for BuildBase: credits are spent per user message (see lib/credits.ts) in place of the hourly message cap.
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // BuildBase: each new user message spends credits before the model runs.
    // Tool-approval continuations carry no new message and cost nothing.
    if (message?.role === "user") {
      const refused = await spendCredits(workspaceId, message.id);
      if (refused) {
        return refused;
      }
    }
