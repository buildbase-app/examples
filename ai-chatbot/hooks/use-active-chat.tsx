// Modified from vercel/chatbot for BuildBase: sends the BuildBase workspace, opens the credit store on a 402 and refreshes the balance.
"use client";
...
export function ActiveChatProvider({ children }: { children: ReactNode }) {
  ...
  // BuildBase: the workspace whose credits pay for each message.
  const { currentWorkspace } = useSaaSWorkspaces();
  const { openCreditStore } = useSaaSAuth();
  const workspaceIdRef = useRef<string | undefined>(currentWorkspace?._id);
  workspaceIdRef.current = currentWorkspace?._id;
  useEffect(() => {
