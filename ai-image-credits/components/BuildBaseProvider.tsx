...
// Added for the BuildBase example: sign-in, the workspace, and its credits.
...
/**
 * Loads the signed-in user's workspaces and selects one; credits belong to a
 * workspace, and the SDK only fetches the list when asked. The first fetch is
 * also what creates a new user's first workspace (with auto-create on), which
 * fires the "Workspace Created" workflow that grants the starting credits.
 */
function WorkspaceLoader() {
  ...
  // Two concurrent fetches can leave the SDK's loading flag stuck, so wait a
  // moment and fetch only if nothing else has.
  useEffect(() => {
    if (!isAuthenticated) {
      return;
    }
