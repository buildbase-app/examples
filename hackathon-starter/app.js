// Modified from sahat/hackathon-starter: sign-in, sign-up, 2FA, passkeys and
// password reset routes are replaced by BuildBase (config/buildbase.js).
/**
 * Module dependencies.
 */
...
const buildbaseController = require('./controllers/buildbase');
...
// Sign-up happens on BuildBase's hosted page, next to sign-in.
app.get('/signup', (req, res) => res.redirect('/login'));
app.get('/auth/buildbase/callback', loginLimiter, userController.getBuildbaseCallback);
...
app.get('/api/buildbase', passportConfig.isAuthenticated, buildbaseController.getBuildBase);
app.post('/api/buildbase/credits', passportConfig.isAuthenticated, buildbaseController.postBuildBaseCredit);
