// Modified from sahat/hackathon-starter: BuildBase handles sign-in, so the
// local strategy is gone and provider OAuth only links accounts.
...
/**
 * OAuth Strategy Overview (modified for the BuildBase example)
 *
 * Signing in is BuildBase's job (see config/buildbase.js). The provider
 * strategies below only LINK a provider account to someone already signed in,
 * so the API examples can call that provider for them.
 *
 * - User is signed in.
 *   - Check if there is an existing account with a provider id.
 *     - If there is, return an error message. (Account merging not supported)
 *     - Else link new OAuth account with currently logged-in user.
 * - User is not signed in: ask them to sign in first.
 */
...
async function handleAuthLogin(req, accessToken, refreshToken, providerName, params, providerProfile, sessionAlreadyLoggedIn, tokenSecret, oauth2provider, tokenConfig = {}, refreshTokenExpiration = null) {
  ...
  // Not signed in: providers link accounts here, they do not create them.
...
function authError2Flash(err, req, done, providerDisplayName) {
  ...
  if (err.message === 'SIGN_IN_FIRST') {
    req.flash('errors', { msg: `Sign in first, then link ${providerDisplayName} from your account page.` });
    done(null, false);
    return true;
  }
  return false;
