...
describe('Passport Config', () => {
  ...
  describe('handleAuthLogin Tests', () => {
    ...
    // Modified for the BuildBase example: signing in is BuildBase's job, so a
    // provider only links to someone already signed in. Upstream's scenarios
    // 3-7 (returning user, email collision, new user, email normalization,
    // missing email) all tested creating or finding an account here.
    it('Scenario 3: not signed in - refuses to sign in or create an account', (done) => {
      req.user = null;
      const providerProfile = { id: 'github-provider-id-1', name: 'Someone', email: 'someone@example.com' };

      _handleAuthLogin(req, 'access-token', 'refresh-token', 'github', { expires_in: 3600 }, providerProfile, false, null, true)
        .then(() => done(new Error('Expected SIGN_IN_FIRST to be thrown')))
        .catch((err) => {
          expect(err.message).to.equal('SIGN_IN_FIRST');
