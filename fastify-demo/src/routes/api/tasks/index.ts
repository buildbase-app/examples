  ...
  fastify.post(
    '/',
    {
      schema: {
        ...
        tags: ['Tasks'],
        description: 'Creates a task and spends one BuildBase credit from your workspace. Send an Idempotency-Key header to make a retry free.'
      }
    },
    async function (request, reply) {
      // Added for the BuildBase example: creating a task is metered. The
      // credit is spent before the insert, so an empty workspace gets a 402
      // and no task. A retried request with the same Idempotency-Key is
      // charged once.
      ...
      try {
        await fastify.buildbase.spend(
          ...
          1,
          'Task created',
          typeof idempotencyKey === 'string' ? idempotencyKey : randomUUID()
        )
      } catch (err) {
        if ((err as { code?: string }).code === 'INSUFFICIENT_CREDITS') {
          reply.code(402)
          return { message: 'Out of credits. Grant some to your workspace in the BuildBase console, or sell them with a credit package.' }
        }
        throw err
      }
