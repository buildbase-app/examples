...
import { build, expectValidationError, fakeBuildBase } from '../../../helper.js'
import {
  Task,
  TaskStatusEnum,
...
async function createUser (
  app: FastifyInstance,
  userData: Partial<{ email: string; username: string }>
) {
...
describe('Tasks api (logged user only)', () => {
  ...
  describe('POST /api/tasks', () => {
    ...
    // Added for the BuildBase example: creating a task spends a credit.
    it('should spend one credit per task, and once per Idempotency-Key', async (t) => {
      const app = await build(t)
      const cookie = await app.login('basic@example.com')
      const create = (key: string) => app.inject({
        method: 'POST',
        url: '/api/tasks',
        payload: { name: 'Metered task' },
        headers: { 'idempotency-key': key },
        cookies: { [app.config.COOKIE_NAME]: cookie }
      })

      assert.strictEqual((await create('key-1')).statusCode, 201)
      assert.strictEqual((await create('key-1')).statusCode, 201)

      assert.strictEqual(
        (app.buildbase as ReturnType<typeof fakeBuildBase>).credits.get('basic@example.com'),
        99
      )
    })

    it('should return 402 and create nothing when the workspace is out of credits', async (t) => {
      const app = await build(t)
      ;(app.buildbase as ReturnType<typeof fakeBuildBase>).credits.set('basic@example.com', 0)
      const before = await app.knex('tasks').count({ n: '*' }).first()

      const res = await app.injectWithLogin('basic@example.com', {
        method: 'POST',
        url: '/api/tasks',
        payload: { name: 'Unpaid task' }
      })

      assert.strictEqual(res.statusCode, 402)
      const after = await app.knex('tasks').count({ n: '*' }).first()
